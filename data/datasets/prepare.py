...
flags.DEFINE_string(
  "out_path", "/fast/najroldi/data/lm/fwedu/test", "Path where to save the dataset.",
)
...
flags.DEFINE_boolean(
  "streaming", False, "Download the dataset in streaming mode. Ignored if `download` is False.",
)
...
flags.DEFINE_integer(
  "seq_length", None, "Sequence length for chunking the dataset. Ignored if `chunk` is False.",
)

flags.DEFINE_boolean(
  "split_train_valid", True, "Split the dataset into train and valid sets after chunking.",
)
flags.DEFINE_integer(
  "n_tokens_valid", None, "Number of tokens in the validation set. Ignored if `chunk` is False.",
)

flags.DEFINE_boolean(
  "save_raw", False, "Save the raw dataset to disk. Ignored if `download` is False.",
)
flags.DEFINE_boolean(
  "save_tokenized", False, "Save the tokenized dataset to disk. Ignored if `tokenize` is False.",
)
flags.DEFINE_boolean(
...
def main(_):
  ...
  if FLAGS.tokenize:
    ...
    tokenized_ds = raw_ds.map(
      partial(tokenize_batched, tokenizer=tokenizer), remove_columns=["text"], **map_setup,
    )
  ...
  if FLAGS.chunk:
    ...
    chunked_ds = tokenized_ds.map(
      partial(concat_chunck, max_seq_length=max_seq_length), **map_setup,
    )
  ...
  if not FLAGS.split_train_valid:
    ...
    train_ds_path = os.path.join(
      out_path, f"tokenized_{tokenizer_name}", f"ctx_{FLAGS.seq_length}", "train",
    )
    valid_ds_path = os.path.join(
      out_path, f"tokenized_{tokenizer_name}", f"ctx_{FLAGS.seq_length}", "valid",
    )
