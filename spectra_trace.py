...
def compute_trace(model, dataloader, seq_len, samples, max_iter, tol):
  """Compute trace. Currently only working in cuda."""
  model.eval()
  ...
  trace_vhv = hessian_comp.trace(maxIter=max_iter, tol=tol)
...
def main(_):
  ...
  for row_idx in row_indices:
    ...
    # Sharpness
    print(f"\tComputing trace")
    trace_vhv = compute_trace(
        model, 
        validloader, 
        cfg.seq_len, 
        FLAGS.samples,
